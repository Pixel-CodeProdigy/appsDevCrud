...
<template>
  <div class="flex justify-between items-center gap-4 p-4 shadow-md">
    <h1 class="text-3xl font-bold text-red-500">Add Student</h1>
    <router-link :to="{ name: 'list' }">
      <button class="w-40 py-2 px-4 bg-green-200">Student List</button>
    </router-link>
  </div>
  <form @submit.prevent="handleAddStucentForm" id="addStudentForm">
    ...
    <div class="p-6">
      <button type="submit" class="w-40 py-2 px-4 bg-green-200">
        Add Student
      </button>
    </div>
  </form>
